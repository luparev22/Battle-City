...
Entity::Entity(Image image, std::list <Entity*> *entities, float x, float y, int x_image, int y_image, int W, int H) :entities(entities),x(x), y(y),W(W),H(H), alive(true) {
...
Tank::Tank(Image image, std::list <Entity*> *entities, float x, float y, int x_image, int y_image, int W, int H) :Entity(image,entities, x, y, x_image, y_image, W, H), speed(100), direction('u'), tankLevel(3) {}

void Tank::shoot(Image image) {
	getEntities()->push_back(new Bullet(this, image, getEntities(), 0, 0, 323, 102, 3, 4));
}

PlayerTank::PlayerTank(Image image, std::list <Entity*> *entities, float x, float y, int x_image, int y_image, int W, int H) : Tank(image,entities, x, y, x_image, y_image, W, H) {
	setName("Player");
	getSprite()->setTextureRect(IntRect(0, 16 * getTankLevel(), 16, 16));
};

EnemyTank::EnemyTank(Image image, std::list <Entity*> *entities, float x, float y, int x_image, int y_image, int W, int H) : Tank(image,entities, x, y, x_image, y_image, W, H) {
	setName("Enemy");
	getSprite()->setTextureRect(IntRect(128, 16 * getTankLevel(), 16, 16));
};

Bullet::Bullet(Tank *player, Image image, std::list <Entity*> *entities, float x, float y, int x_image, int y_image, int W, int H) :Entity(image, entities, x, y, x_image, y_image, W, H) {
	setName("Bullet");
...
void EnemyTank::update(float dt) {
	if (rand() % 4096 == 0 && !getReload())this->shoot(getTexture().copyToImage());
